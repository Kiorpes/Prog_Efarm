...
$allowed_columns = ['start', 'end', 'name', 'room'];
$allowed_directions = ['asc', 'desc'];

$sort = test_input($_GET['sort'] ?? 'room');
...
$countResult = $conn->query("SELECT COUNT(*) AS total FROM reservations");
...
$stmt = $conn->prepare("SELECT reservations.start,
						reservations.end,
						reservations.name,
...
<html lang="en">
<head>
  <title>Hotel Verdania: Reservation Log</title>
  <meta charset="utf-8">
...
<table class="table table-hover table-warning table-striped text-center">
    <thead class="thead">
        <tr>
            <th width="20%">
                <a href="?page=<?= $page ?>&sort=start&order=<?= $order === 'asc' ? 'desc' : 'asc' ?>">
                    Start Date <?= sort_arrow('start', $sort, $order) ?>
                </a>
            </th>
            <th width="20%">
                <a href="?page=<?= $page ?>&sort=end&order=<?= $order === 'asc' ? 'desc' : 'asc' ?>">
                    End Date <?= sort_arrow('end', $sort, $order); ?>
                </a>
            </th>
            <th width="30%">
                <a href="?page=<?= $page ?>&sort=name&order=<?= $order === 'asc' ? 'desc' : 'asc' ?>">
                    Person Name <?= sort_arrow('name', $sort, $order); ?>
                </a>
            </th>
            <th width="30%">
                <a href="?page=<?= $page ?>&sort=room&order=<?= $order === 'asc' ? 'desc' : 'asc' ?>">
                    Room <?= sort_arrow('room', $sort, $order); ?>
                </a>
            </th>
        </tr>
    </thead>
