...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if(empty($errors)) {
		require "db.php";
		$stmt = $conn->prepare("UPDATE rooms SET name = ?, accomodations = ?, price = ?, description = ?, image_path = ? WHERE id = ?");
		$stmt->bind_param("ssissi", $name, $accomodations, $price, $description, $image, $id);
		try{
			$stmt->execute();
			$_SESSION['Notification'] = "Room Updated Successfully!";
...
<html lang="en">
<head>
  <title>Hotel Verdania: Update Room</title>
  <meta charset="utf-8">
...
<div class="container" id="main">
  <div class="jumbotron text-center">
	<h1>Update Room</h1>
	<p>Update the '<?= $name ?>' room's data:</p> 
  </div>
	...
	<div class="row">
	  <div class="col-md-8">
        <div class="form-group">
            ...
            <input type="textarea" class="form-control" name="description" value="<?= $description ?>" required>
        </div>
	 </div>
	</div>
	<div class="row">
	  <div class="col-md-8">
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="accomodations" value="<?= $accomodations ?>" required>
        </div>
	 </div>
	</div>
	<div class="row">
